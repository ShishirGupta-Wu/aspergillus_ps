...
foreach my $file (@store_array) {
  ...
  open (OUT, ">$name.H0.ctl")|| die "can't open contral file:$!\n";

  print OUT "seqfile           = $seqdir/$name.phylip\n",
            "treefile          = $tree\n",
            "outfile           = $outputdir/$name-BSM-H0\n",
            "noisy             = 0\n",
            "verbose           = 1\n",
            "runmode           = 0\n",
            "seqtype           = 1\n",
...
=head1 DESCRIPTION
	...
	Run a null hypothesis model H0 using the codeml program on many coding
    genes.
